...
namespace plume {
    ...
    struct RenderCommandList {
        ...
        // REXGLUE: GPU progress breadcrumb (WriteBufferImmediate on D3D12);
        // no-op on backends without support. dstBuffer must be in a
        // copy-dest-compatible state (readback heap buffers always are).
        virtual void writeBufferImmediate(const RenderBuffer *dstBuffer, uint64_t dstOffset, uint32_t value) { }
