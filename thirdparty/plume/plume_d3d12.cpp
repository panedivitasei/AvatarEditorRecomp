...
#   endif
        ...
        case RenderDescriptorRangeType::TEXTURE: {
            if ((nativeResource != nullptr) && (textureView != nullptr)) {
                ...
                case RenderTextureViewDimension::TEXTURE_2D:
                    ...
                    break;
                // REXGLUE FIX: forced 2D-array SRV (legal with ArraySize 1) so
                // plain 2D textures can back Texture2DArray declarations.
                case RenderTextureViewDimension::TEXTURE_2D_ARRAY:
                    if (isMSAA) {
                        srvDesc.ViewDimension = D3D12_SRV_DIMENSION_TEXTURE2DMSARRAY;
                        srvDesc.Texture2DMSArray.FirstArraySlice = interfaceTextureView->arrayIndex;
                        srvDesc.Texture2DMSArray.ArraySize = std::max(interfaceTextureView->arraySize, 1U);
                    }
                    else {
                        srvDesc.ViewDimension = D3D12_SRV_DIMENSION_TEXTURE2DARRAY;
                        srvDesc.Texture2DArray.MipLevels = interfaceTextureView->mipLevels;
                        srvDesc.Texture2DArray.MostDetailedMip = interfaceTextureView->mipSlice;
                        srvDesc.Texture2DArray.FirstArraySlice = interfaceTextureView->arrayIndex;
                        srvDesc.Texture2DArray.ArraySize = std::max(interfaceTextureView->arraySize, 1U);
                    }
        ...
        case RenderDescriptorRangeType::READ_WRITE_TEXTURE: {
            if ((nativeResource != nullptr) && (textureView != nullptr)) {
                ...
                case RenderTextureViewDimension::TEXTURE_2D:
                    ...
                    break;
                // REXGLUE FIX: forced 2D-array UAV (legal with ArraySize 1).
                case RenderTextureViewDimension::TEXTURE_2D_ARRAY:
                    uavDesc.ViewDimension = D3D12_UAV_DIMENSION_TEXTURE2DARRAY;
                    uavDesc.Texture2DArray.MipSlice = interfaceTextureView->mipSlice;
                    uavDesc.Texture2DArray.FirstArraySlice = interfaceTextureView->arrayIndex;
                    uavDesc.Texture2DArray.ArraySize = std::max(interfaceTextureView->arraySize, 1U);
                    break;
                case RenderTextureViewDimension::TEXTURE_3D:
    ...
    D3D12SwapChain::D3D12SwapChain(D3D12CommandQueue *commandQueue, const RenderSwapChainDesc &desc) {
        ...
        // REXGLUE DEBUG BISECT: match the (known-working) rex D3D12 presenter
        // swapchain desc — no waitable object, AlphaMode IGNORE.
        swapChainFlags = 0;
    ...
    bool D3D12SwapChain::present(uint32_t textureIndex, RenderCommandSemaphore **waitSemaphores, uint32_t waitSemaphoreCount) {
        ...
        // REXGLUE: first present failure dumps hr + device-removed reason +
        // DRED breadcrumbs (the exact command that killed the driver) to
        // plume_present_dbg.txt — the cross-hardware TDR forensics record.
        if (FAILED(res)) {
            static bool failureDumped = false;
            if (!failureDumped) {
                failureDumped = true;
                ID3D12Device8 *dev = commandQueue->device->d3d;
                HRESULT removed = dev ? dev->GetDeviceRemovedReason() : E_FAIL;
                FILE *f = fopen("plume_present_dbg.txt", "a");
                if (f != nullptr) {
                    fprintf(f, "PRESENT FAILED hr=%08lX deviceRemovedReason=%08lX\n",
                            res, removed);
                    // Debug-layer info queue: the stored validation messages
                    // leading up to the removal (present when
                    // REXGLUE_D3D_DEBUG armed the layer).
                    ID3D12InfoQueue *iq = nullptr;
                    if (dev != nullptr && SUCCEEDED(dev->QueryInterface(IID_PPV_ARGS(&iq)))) {
                        const UINT64 stored = iq->GetNumStoredMessages();
                        const UINT64 from = stored > 40 ? stored - 40 : 0;
                        fprintf(f, "InfoQueue: %llu stored messages, dumping last %llu:\n",
                                (unsigned long long)stored, (unsigned long long)(stored - from));
                        for (UINT64 mi = from; mi < stored; mi++) {
                            SIZE_T len = 0;
                            iq->GetMessage(mi, nullptr, &len);
                            if (len == 0 || len > 4096) continue;
                            D3D12_MESSAGE *msg = (D3D12_MESSAGE *)malloc(len);
                            if (msg != nullptr) {
                                if (SUCCEEDED(iq->GetMessage(mi, msg, &len))) {
                                    fprintf(f, "  [sev%d id%d] %s\n", int(msg->Severity),
                                            int(msg->ID), msg->pDescription ? msg->pDescription : "?");
                                }
                                free(msg);
                            }
                        }
                    ...
                    if (dev != nullptr && SUCCEEDED(dev->QueryInterface(IID_PPV_ARGS(&dred)))) {
                        ...
                        if (SUCCEEDED(dred->GetAutoBreadcrumbsOutput(&bc))) {
                            ...
                            int ni = 0;
                            while (n != nullptr && ni < 32) {
                                UINT last = n->pLastBreadcrumbValue ? *n->pLastBreadcrumbValue : 0;
                                fprintf(f, "DRED node %d: cmdlist=%ls count=%u lastExecuted=%u\n",
                                        ni,
                                        n->pCommandListDebugNameW ? n->pCommandListDebugNameW : L"?",
                                        n->BreadcrumbCount, last);
                                if (n->pCommandHistory != nullptr && last < n->BreadcrumbCount) {
                                    UINT from = last > 8 ? last - 8 : 0;
                                    UINT to = (last + 4 < n->BreadcrumbCount) ? last + 4 : n->BreadcrumbCount;
                                    for (UINT i = from; i < to; i++) {
                                        fprintf(f, "  op[%u]=%d%s\n", i, int(n->pCommandHistory[i]),
                            ...
                            if (ni == 0) {
                                fprintf(f, "DRED: no breadcrumb nodes (enable with REXGLUE_DRED=1)\n");
                            }
                        }
                        D3D12_DRED_PAGE_FAULT_OUTPUT pf = {};
                        if (SUCCEEDED(dred->GetPageFaultAllocationOutput(&pf))) {
                            fprintf(f, "DRED pagefault VA=%016llX\n",
                                    (unsigned long long)pf.PageFaultVA);
                            // Allocation lists NAME the owner of the faulting
                            // VA: existing = live resource, recent-freed =
                            // USE-AFTER-FREE (the frame-count-based texture
                            // retirement outrunning a slow GPU's queue).
                            const D3D12_DRED_ALLOCATION_NODE *an = pf.pHeadExistingAllocationNode;
                            int ai = 0;
                            while (an != nullptr && ai < 16) {
                                fprintf(f, "  existing[%d]: type=%d name=%s\n", ai,
                                        int(an->AllocationType),
                                        an->ObjectNameA ? an->ObjectNameA : "?");
                                an = an->pNext;
                                ai++;
                            }
                            an = pf.pHeadRecentFreedAllocationNode;
                            ai = 0;
                            while (an != nullptr && ai < 16) {
                                fprintf(f, "  RECENT-FREED[%d]: type=%d name=%s\n", ai,
                                        int(an->AllocationType),
                                        an->ObjectNameA ? an->ObjectNameA : "?");
                                an = an->pNext;
                                ai++;
        ...
        // REXGLUE DEBUG: DXGI ground truth — is DWM actually displaying these
        // presents? PresentRefreshCount only advances for displayed frames.
        {
            static int dbgCounter = 0;
            if ((dbgCounter++ % 300) == 0) {
                DXGI_FRAME_STATISTICS stats = {};
                HRESULT statsRes = d3d->GetFrameStatistics(&stats);
                UINT lastPresent = 0;
                d3d->GetLastPresentCount(&lastPresent);
                FILE *dbgFile = fopen("plume_present_dbg.txt", "a");
                if (dbgFile != nullptr) {
                    fprintf(dbgFile, "present#%d hr=%08lX statsHr=%08lX PresentCount=%u RefreshCount=%u SyncRefresh=%u lastPresent=%u sync=%u\n",
                        dbgCounter, res, statsRes, stats.PresentCount, stats.PresentRefreshCount, stats.SyncRefreshCount, lastPresent, syncInterval);
                    fclose(dbgFile);
                }
            }
        }
    ...
    bool D3D12SwapChain::resize() {
        ...
        for (uint32_t i = 0; i < desc.textureCount; i++) {
            // REXGLUE FIX: null-guard so a retried resize after a failed
            // ResizeBuffers (which leaves the textures released and nulled)
            // does not call Release() through a null pointer.
            if (textures[i].d3d != nullptr) {
                textures[i].d3d->Release();
                textures[i].d3d = nullptr;
            }
...
#   endif
    ...
    void D3D12CommandList::writeBufferImmediate(const RenderBuffer *dstBuffer, uint64_t dstOffset, uint32_t value) {
        // REXGLUE: GPU progress breadcrumb. MARKER_OUT = the write lands only
        // after ALL previous commands complete through the pipeline (DEFAULT
        // is normal copy ordering and can land before a preceding draw
        // finishes — would over-report progress on a hang). The CPU reads the
        // mapped readback buffer live.
        if (d3dV4 == nullptr || dstBuffer == nullptr) {
            return;
        }
        const D3D12Buffer *interfaceBuffer = static_cast<const D3D12Buffer *>(dstBuffer);
        D3D12_WRITEBUFFERIMMEDIATE_PARAMETER param;
        param.Dest = interfaceBuffer->d3d->GetGPUVirtualAddress() + dstOffset;
        param.Value = value;
        const D3D12_WRITEBUFFERIMMEDIATE_MODE mode = D3D12_WRITEBUFFERIMMEDIATE_MODE_MARKER_OUT;
        d3dV4->WriteBufferImmediate(1, &param, &mode);
    }
...
#   endif
    ...
    void D3D12CommandList::copyTextureRegion(const RenderTextureCopyLocation &dstLocation, const RenderTextureCopyLocation &srcLocation, uint32_t dstX, uint32_t dstY, uint32_t dstZ, const RenderBox *srcBox) {
        ...
        // REXGLUE FIX: texture-to-buffer copies (readback) have a null dst
        // texture; upstream assumed texture destinations only.
...
#       endif
    ...
    D3D12PipelineLayout::D3D12PipelineLayout(D3D12Device *device, const RenderPipelineLayoutDesc &desc) {
        ...
        for (uint32_t i = 0; i < desc.rootDescriptorDescsCount; i++) {
            ...
            D3D12_ROOT_PARAMETER rootParameter = {};
            rootParameter.ShaderVisibility = toD3D12(rootDescriptorDesc.shaderVisibility);
    ...
    D3D12Device::D3D12Device(D3D12Interface *renderInterface, const std::string &preferredDeviceName) {
        ...
        while (renderInterface->dxgiFactory->EnumAdapters1(adapterIndex++, &adapterOption) != DXGI_ERROR_NOT_FOUND) {
            ...
            // REXGLUE: adapter pick (env REXGLUE_ADAPTER): hex vendor id
            // ("8086" = Intel) or case-insensitive name fragment ("intel",
            // "vega") — hybrid-GPU machines choose the GPU per config
            // (Hades Canyon Intel-vs-Vega repro of the Intel-fleet bugs).
            // The caller falls back to unfiltered if nothing matches.
            {
                char adapterEnv[64] = {};
                if (GetEnvironmentVariableA("REXGLUE_ADAPTER", adapterEnv, sizeof(adapterEnv)) && adapterEnv[0] != 0) {
                    ...
                    bool matched = (hexEnd != adapterEnv && *hexEnd == 0 &&
                                    vendorFilter != 0 && adapterDesc.VendorId == vendorFilter);
                    if (!matched) {
                        std::string name = Utf16ToUtf8(adapterDesc.Description);
                        std::string frag = adapterEnv;
                        for (char &c : name) c = char(tolower((unsigned char)c));
                        for (char &c : frag) c = char(tolower((unsigned char)c));
                        matched = name.find(frag) != std::string::npos;
                    }
                    if (!matched) {
                        adapterOption->Release();
                        continue;
            ...
            // REXGLUE: DRED (device-removed breadcrumbs + page-fault data),
            // enabled via env REXGLUE_DRED=1 before device creation — on a
            // TDR the present/resize failure path dumps the exact command
            // that killed the driver (cross-hardware forensics).
            {
                char dredEnv[8] = {};
                if (GetEnvironmentVariableA("REXGLUE_DRED", dredEnv, sizeof(dredEnv)) && dredEnv[0] == '1') {
...
#   endif
    ...
    std::unique_ptr<RenderPipeline> D3D12Device::createGraphicsPipeline(const RenderGraphicsPipelineDesc &desc) {
        ...
        // REXGLUE fix: CreateGraphicsPipelineState failure left a wrapper
        // with a null PSO — setPipeline then crashed inside D3D12Core.
        // A failed pipeline must be a null result the caller can test.
...
#   endif
        ...
        // REXGLUE: runtime debug layer (env REXGLUE_D3D_DEBUG: 1 = layer,
        // 2 = layer + GPU-based validation). Needs the Graphics Tools
        // optional feature; fails gracefully without it. The present-failure
        // dump drains the info queue — the validation error that precedes a
        // device removal names the exact invalid access.
        {
            char dbgEnv[8] = {};
            if (GetEnvironmentVariableA("REXGLUE_D3D_DEBUG", dbgEnv, sizeof(dbgEnv)) && dbgEnv[0] != 0 && dbgEnv[0] != '0') {
                ID3D12Debug1 *rexDebug = nullptr;
                if (SUCCEEDED(D3D12GetDebugInterface(IID_PPV_ARGS(&rexDebug)))) {
                    rexDebug->EnableDebugLayer();
                    if (dbgEnv[0] == '2') {
                        rexDebug->SetEnableGPUBasedValidation(TRUE);
                    }
                    dxgiFactoryFlags |= DXGI_CREATE_FACTORY_DEBUG;
                    rexDebug->Release();
                    FILE *df = fopen("plume_present_dbg.txt", "a");
                    if (df != nullptr) {
                        fprintf(df, "D3D12 DEBUG LAYER ENABLED (mode %c)\n", dbgEnv[0]);
                        fclose(df);
                    }
                }
            }
