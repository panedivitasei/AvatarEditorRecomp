...
namespace plume {
    ...
    enum class RenderTextureViewDimension {
        ...
        TEXTURE_3D,
        TEXTURE_CUBE,
        // REXGLUE FIX: explicit 2D-array view even when arraySize == 1, so a
        // plain 2D texture can be sampled through a Texture2DArray HLSL
        // declaration (the Xenos bindless contract binds all 2D textures as
        // arrays). TEXTURE_2D only becomes an array view when arraySize > 1.
        TEXTURE_2D_ARRAY
    };
    ...
    struct RenderTextureViewDesc {
        ...
        // REXGLUE FIX: forced 2D-array view (valid with arraySize == 1).
        static RenderTextureViewDesc Texture2DArray(RenderFormat format, uint32_t arraySize = 1, uint32_t arrayIndex = 0) {
            RenderTextureViewDesc viewDesc;
            viewDesc.format = format;
            viewDesc.dimension = RenderTextureViewDimension::TEXTURE_2D_ARRAY;
            viewDesc.arraySize = arraySize;
            viewDesc.arrayIndex = arrayIndex;
            return viewDesc;
        }
    ...
    struct RenderRootDescriptorDesc {
        ...
        // Restricting visibility to one stage allows two root descriptors to
        // alias the same register/space with different per-stage contents.
        RenderShaderVisibility shaderVisibility = RenderShaderVisibility::ALL;

        RenderRootDescriptorDesc() = default;

        RenderRootDescriptorDesc(uint32_t shaderRegister, uint32_t registerSpace, RenderRootDescriptorType type, RenderShaderVisibility shaderVisibility = RenderShaderVisibility::ALL) {
