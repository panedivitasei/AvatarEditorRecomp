...
namespace plume {
    ...
    struct RenderPipelineLayoutBuilder {
        ...
        uint32_t addRootDescriptor(uint32_t shaderRegister, uint32_t registerSpace, RenderRootDescriptorType type, RenderShaderVisibility shaderVisibility = RenderShaderVisibility::ALL) {
            assert(open && "Builder must be open.");

            uint32_t returnValue = layoutDesc.rootDescriptorDescsCount;
            rootDescriptorDescs.emplace_back(shaderRegister, registerSpace, type, shaderVisibility);
