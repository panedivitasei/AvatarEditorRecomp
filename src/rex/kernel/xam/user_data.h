...
#ifndef REX_KERNEL_XAM_USER_DATA_H_
#define REX_KERNEL_XAM_USER_DATA_H_
...
// Forward declaration; full class lives in user_property.h.
class Property;

// Reads a null-terminated, byte-swapped UTF-16 string (replaces
// string_util::read_u16string_and_swap).
inline std::u16string read_u16string_and_swap(const char16_t* str_begin) {
  std::u16string result;
  for (const char16_t* p = str_begin; *p != 0; ++p) {
    result.push_back(
        static_cast<char16_t>(rex::byte_swap(static_cast<uint16_t>(*p))));
  }
  return result;
}
 ...
 public:
  ...
  UserDataTypes get_host_data() const {
    ...
    if (data_.type == X_USER_DATA_TYPE::WSTRING) {
      ...
      return rex::kernel::xam::read_u16string_and_swap(str_begin);
    }
