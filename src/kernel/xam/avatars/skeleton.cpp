...
void Skeleton::Initialize() {
  ...
  for (uint8_t i = joint_count - 1; i >= 1; --i) {
    ...
    // The parent quaternion must carry its real w: mirrored bones sit at
    // w ~ 0, and forcing w to 1.f composes a non-unit quaternion, which
    // corrupts the local rest pose of every joint below it.
    const glm::quat parentQ(parentJoint.bindpose.rotation.w,
                            parentJoint.bindpose.rotation.x,
...
std::shared_ptr<Skeleton> Skeleton::Load(const uint8_t* strb_buffer,
  ...
  if (!compression::GetUncompressedSize(compressed_buffer, compressed_size,
                                        data_size)) {
    assert_always();
    REXKRNL_ERROR("Failed to get uncompressed size for avatar skeleton!");
    return nullptr;
  }
  ...
  if (!compression::Decompress(compressed_buffer, compressed_size,
    ...
    assert_always();
    REXKRNL_ERROR("Failed to decompress avatar skeleton!");
    return nullptr;
  }
