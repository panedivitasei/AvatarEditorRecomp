...
BlendShapeIndexPatch BlendShapeIndexPatch::Read(BitStream& stream) {
  ...
  // The index list is byte-aligned in the container. With the common 12-bit
  // index width an odd index count ends mid-byte, so without this realign the
  // vertex patch that follows is read bit-skewed.
...
bool BlendShapeTargetMatches(const AssetId& target, const AssetId& asset_id) {
  ...
  // Body-hiding shapes reference the stock body population (a=2) with an
  // inconsistent b field across sources: the worn female body component
  // carries the pack index (b=1), while marketplace shapes and some stock
  // pack shapes use b=0. The gender lives in c and ApplyBlendShape's
  // buffer-size checks reject any wrong-body pairing, so ignore b when both
  // sides are body ids.
  if (target.a.get() == 2u && asset_id.a.get() == 2u) {
    return target.c.get() == asset_id.c.get() &&
           !std::memcmp(target.d, asset_id.d, sizeof(target.d));
  }
  return false;
}

bool BlendShape::matches(const AssetId& assetId) const {
  return BlendShapeTargetMatches(index_patch.original_asset_id, assetId) &&
         BlendShapeTargetMatches(vertex_patch.original_asset_id, assetId);
}
