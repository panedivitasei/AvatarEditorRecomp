...
bool LoadAnimationToGuest(const AssetId& asset_id,
  ...
  {
    const auto& tfs = animation->texture_frame_set;
    std::string s;
    for (size_t i = 0; i < countof(tfs.element_serializers); ++i) {
      const auto& ls = tfs.element_serializers[i].layer_index_serializer;
      s += fmt::format(" e{}:bits={},base={}", i, ls.bit_count, ls.base_value);
    }
    uint32_t head[2] = {0, 0};
    const auto& cb = animation->compressed_data_bytes;
    const size_t off = animation->textures_byte_offset;
    if (cb.size() >= off + 8) std::memcpy(head, cb.data() + off, 8);
  }
...
bool LoadAnimationToGuest(AssetPack* asset_pack, const AssetId& asset_id,
  ...
  if (!asset_pack->GetAssetData(asset_id, strb_buffer, strb_size)) {
    REXKRNL_ERROR("Failed to find avatar animation {}!", asset_id.to_string());
    return false;
  }
  ...
  if (!animation) {
    REXKRNL_ERROR("Failed to load avatar animation {}!", asset_id.to_string());
    return false;
  }
