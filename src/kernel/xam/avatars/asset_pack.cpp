...
bool LoadAssetPackFromMemoryV1(const uint8_t* data, size_t data_size,
  ...
  if (data_size < header_size) {
    REXKRNL_ERROR("avatar asset pack V1: cannot load file header, buffer too small");
    return false;
  }
  ...
  if (data_size < header_size) {
    REXKRNL_ERROR("avatar asset pack V1: cannot load asset headers, buffer too small");
    return false;
  }
  ...
  for (size_t i = 0; i < header.asset_count; ++i) {
    ...
    if (data_size < asset_data_offset + asset_data_size) {
      REXKRNL_ERROR("avatar asset pack V1: cannot load asset #{}, buffer too small",
             i);
...
bool LoadAssetPackFromMemoryV2(const uint8_t* data, size_t data_size,
  ...
  if (data_size < header_size) {
    REXKRNL_ERROR("avatar asset pack V2: cannot load file header, buffer too small");
    return false;
  }
  ...
  if (data_size < header_size) {
    REXKRNL_ERROR("avatar asset pack V2: cannot load asset headers, buffer too small");
    return false;
  }
  ...
  for (size_t i = 0; i < header.asset_count; ++i) {
    ...
    auto asset_data_size = size_t(asset_header.data_size);
    if (data_size < asset_data_offset + asset_data_size) {
      REXKRNL_ERROR("avatar asset pack V2: cannot load asset #{}, buffer too small",
             i);
      asset_infos.push_back({});
      continue;
    }
...
bool LoadAssetPackFromMemoryV3(const uint8_t* data, size_t data_size,
  ...
  if (data_size < header_size) {
    REXKRNL_ERROR("avatar asset pack V3: cannot load file header, buffer too small");
    return false;
  }
  ...
  if (data_size < header_size) {
    REXKRNL_ERROR("avatar asset pack V3: cannot load asset headers, buffer too small");
    return false;
  }
  ...
  for (size_t i = 0; i < header.asset_count; ++i) {
    ...
    auto asset_data_size = size_t(asset_header.data_size);
    if (data_size < asset_data_offset + asset_data_size) {
      REXKRNL_ERROR("avatar asset pack V3: cannot load asset #{}, buffer too small",
             i);
      asset_infos.push_back({});
      continue;
    }
...
bool LoadAssetPackFromMemory(const void* buffer, size_t buffer_size,
                             std::vector<AssetInfo>& asset_infos) {
  if (buffer_size < 16) {
    REXKRNL_ERROR("Cannot load avatar asset pack: buffer too small for version");
    return false;
  }
  ...
  size_t version = 0;
  for (size_t i = 0; i < rex::countof(version_guids); ++i) {
  ...
  if (!version) {
    REXKRNL_ERROR("Cannot load avatar asset pack: unknown version");
    return false;
  }
  ...
  REXKRNL_ERROR("Cannot load avatar asset pack: unsupported version {}", version);
  return false;
}
...
bool AssetPack::GetAssetDataByIndex(size_t index, const uint8_t*& buffer,
                                    size_t& size) const {
  if (index >= asset_infos_.size()) {
    return false;
  }
...
std::u16string AssetPack::GetAssetName(AssetId id) const {
  return GetAssetNameByIndex(id.b);
}

std::u16string AssetPack::GetAssetNameByIndex(size_t index) const {
  if (index >= asset_infos_.size()) {
